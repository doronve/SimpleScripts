select
    ...
    c.alias,
    a.node_name,
    case when (100-a.percentage_idle) > 100 then 100 else (100-a.percentage_idle) END as cpu_utilization,
    100-b.available*100/b.total as ram_utilization,
    to_char(a.snaptime::timestamp(0), 'DD/MM/YYYY HH24:MI:SS')
from
    cpu a ,
    free b ,
    pci_tools_cluster_monitor c
where
    (a.cluster_name,a.snaptime)
in
    (select cluster_name,snaptime from last_snaptime)
