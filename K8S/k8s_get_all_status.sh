#!/bin/bash
#
# Name: k8s_get_all_status.sh
#

myhomefile=/var/www/html/myhome_k8s.html
...
let i=0
#---
for k8shost in $(cat /BD/Monitor/node*lst | grep k8s | sort -u | grep -x -v -f /BD/Monitor/nodeexeptions.lst | grep -- -1)
do
  let i=${i}+1
  ...
  echo "<td>$(bash K8S/k8s_get_all_pods.sh     $k8shost)</td>"                               >> ${myhomefile}.new
  echo "</tr>" >> ${myhomefile}.new
done
